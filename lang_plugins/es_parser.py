...
from schemas.base import SpanishFeatures, ValueField, Provenance, Inflection, PosType
...
def parse_spanish_data(raw_data: RawSpanishData) -> SpanishFeatures:
    ...
    number_vf = _create_value_field(raw_data.number, "number", provenance, raw_data.confidence_map)
    part_of_speech_vf = _create_value_field(raw_data.pos, "pos", provenance, raw_data.confidence_map)
    verb_conjugations = [Inflection(form=_create_value_field(form, tense, provenance, raw_data.confidence_map), type=tense) for tense, form in raw_data.verb_conjugations.items()] if raw_data.verb_conjugations else []
